...
**21/07/2026 — Migración completada**: `barberiadelpuerto.es` comprado y en producción.
- Revisado antes de migrar: ni `citabotapp-barberia-puerto` ni `barberia-puerto-api` referencian `labarberiadelpuerto.cuxy.app` en el código (solo salía en documentación) — confirma que la URL nunca se compartió realmente con clientes.
- Verificación de email del contacto registrante (ICANN) bloqueó la propagación varias horas — hay que revisar el correo inmediatamente después de comprar cualquier dominio nuevo en Namecheap y confirmar ese enlace cuanto antes, si no el dominio no llega a activarse.
- Cloudflare: dominio conectado como "Connect a domain" (no transferido, sigue registrado en Namecheap) → nameservers de Cloudflare puestos en Namecheap → una vez activa la zona, añadido como Custom Domain en el Worker `labarberiadelpuerto` (Workers & Pages → Domains → Add Domain).
- `barberiadelpuerto.es` responde HTTP 200 y sirve la web correctamente.
- **Sin redirección** desde `labarberiadelpuerto.cuxy.app`: al no haberse compartido nunca esa URL, se decidió no crear la regla de redirección. Si en el futuro aparece alguien con el enlace viejo guardado, añadir una Redirect Rule en el dominio `cuxy.app` (`labarberiadelpuerto.cuxy.app` → `barberiadelpuerto.es`, 301).
